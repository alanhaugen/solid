...
void IComponent::SetUniforms(Array<std::shared_ptr<UniformData> > *uniforms_)
{
    delete uniforms;

    if (uniforms_ == NULL)
    {
        uniforms = new Array<std::shared_ptr<UniformData> >();
    }
    else
    {
        uniforms = uniforms_;
...
{
    uniforms->Add(std::shared_ptr<UniformData>(new UniformData(location, uniform)));
}
